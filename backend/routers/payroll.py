...
def _get_att(db: Session, emp_id: int, month: int, year: int) -> list:
    """Fetch all attendance records for employee in the given month/year."""
...
def calculate(
    ...
    """
    Calculate (or recalculate) payroll for one employee for a given month.

    FIX: No longer raises 422 when zero attendance records exist.
    Payroll can be calculated with 0 present days — the result is
    a net_pay of 0 minus any advance/deposit. This is valid.

    FIX: calc_monthly_payroll now returns only Payroll model columns,
    so Payroll(**result) no longer crashes with InvalidRequestError
    from extra keys like per_day / per_hr.
    """
    ...
    # Fetch attendance — zero records is allowed
    att_records = _get_att(db, payload.employee_id, payload.month, payload.year)

    # Calculate — returns only Payroll model column keys
    try:
    ...
    try:
        ...
        pr = Payroll(
            ...
            month=payload.month,
            year=payload.year,
            **result,            # safe — result has no extra keys
        )
        db.add(pr)
        db.commit()
        db.refresh(pr)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Database error saving payroll: {str(e)}"
        )
...
# ─────────────────────────────────────────────────────────────
# Export: full monthly Excel (all employees)
# ─────────────────────────────────────────────────────────────
...
def export_monthly_excel(
    ...
    hdrs = ["#","Name","Desig","Dept","Basic","Per Day","Per Hr",
            "Present","Absent","OT Hrs","Less Hrs","Out Hrs","Gross",
            "Extra Pay","Less Ded","Advance","Deposit","PF","Incentives","Net Pay"]
    ...
    for idx, emp in enumerate(employees, 1):
        ...
        vals    = [idx, emp.name, emp.designation or "", emp.department or "",
                   ...
                   pr.present_days     if pr else 0,
                   pr.absent_days      if pr else 0,
                   pr.total_ot_hours   if pr else 0,
                   pr.total_less_hours if pr else 0,
                   pr.total_out_hours  if pr else 0,
                   pr.gross_salary     if pr else 0,
                   pr.extra_pay        if pr else 0,
                   pr.less_deduction   if pr else 0,
                   pr.advance          if pr else 0,
                   pr.deposit          if pr else 0,
                   pr.pf_deduction     if pr else 0,
                   pr.incentives       if pr else 0,
                   pr.net_pay          if pr else 0]
        for col, val in enumerate(vals, 1):
            c = ws.cell(row=row, column=col, value=val)
            c.fill   = fill("D1FAE5") if col == 20 else fill(bg)
            c.border = thin(); c.alignment = ctr; c.font = Font(size=8)
            if col in [5,6,7,13,14,15,16,17,18,19,20]:
                c.number_format = "#,##0.00"

    for i, w in enumerate([4,22,8,10,10,9,9,7,7,7,7,7,11,9,9,9,9,8,9,11], 1):
        ws.column_dimensions[get_column_letter(i)].width = w
    ws.freeze_panes = "A3"

    # Per-employee detail sheets
    for emp in employees:
        ws2     = wb.create_sheet(title=emp.name[:28])
        pr      = db.query(Payroll).filter(Payroll.employee_id == emp.id,
                  Payroll.month == month, Payroll.year == year).first()
        ...
        c = ws2["A1"]
        c.value = f"SALARY SHEET   {emp.name.upper()}   {calendar.month_name[month]} {year}"
        c.font  = Font(bold=True, size=11, color="FFFFFF")
        c.fill  = fill("1E293B"); c.alignment = ctr
        ws2.row_dimensions[1].height = 22

        hdrs2 = ["DATE","IN TIME","OUT TIME","TOTAL TIME","+","-","TIME DIFF","DAY",
        ...
        for day in range(1, num_days + 1):
            ...
            extra_p = round(ot_hrs  * per_hr, 2)
            less_p  = round((less_hrs + out_hrs) * per_hr, 2)
            net_d   = round(col_u + extra_p - less_p, 2)
            t_str   = f"{int(tot_hrs)}:{int((tot_hrs % 1)*60):02d}" if tot_hrs else ""

            vals2 = [d.strftime("%d-%m-%Y"), in_t, out_t, t_str,
                     ...
                     present, 1-present, 0, 0, ot_hrs, less_hrs, out_hrs,
                     col_u, "", "", 0, extra_p, less_p, net_d]
            for col, val in enumerate(vals2, 1):
                ...
                if col in [21,22,23,24,25,26,27]:
                    c.number_format = "#,##0.00"
            ws2.row_dimensions[row].height = 13

        if pr:
            sr = num_days + 3
            sv = {14: pr.present_days, 15: pr.absent_days,
                  18: pr.total_ot_hours, 19: pr.total_less_hours,
                  20: pr.total_out_hours, 21: pr.gross_salary,
                  22: pr.advance, 23: pr.deposit, 24: pr.pf_deduction,
                  25: pr.extra_pay, 26: pr.less_deduction, 27: pr.net_pay}
            ws2.cell(row=sr, column=1, value="TOTALS").font = Font(bold=True, size=8)
            for col, val in sv.items():
                c = ws2.cell(row=sr, column=col, value=val)
                c.font   = Font(bold=True, size=8, color="065F46" if col == 27 else "000000")
                c.fill   = fill("E0F2FE"); c.border = thin()
                c.alignment = ctr; c.number_format = "#,##0.00"

        for i, w in enumerate([12,8,8,9,6,6,7,10,5,5,8,8,8,6,6,5,6,6,6,6,9,8,8,6,8,8,10], 1):
...
# ─────────────────────────────────────────────────────────────
# Export: PDF payslip for one employee
# ─────────────────────────────────────────────────────────────
...
def export_payslip_pdf(
    ...
    if not pr:
        raise HTTPException(404,
            f"Payroll not calculated yet for {calendar.month_name[month]} {year}. "
            "Go to Payroll page and click Calculate first.")

    buf   = _build_pdf(emp, pr, month, year)
    fname = f"Payslip_{emp.name.replace(' ','_')}_{calendar.month_name[month]}_{year}.pdf"
    return StreamingResponse(buf, media_type="application/pdf",
        headers={"Content-Disposition": f"attachment; filename={fname}"})


def _build_pdf(emp, pr, month: int, year: int) -> io.BytesIO:
    ...
    t_s  = ps("T",  fontSize=16, textColor=WHITE, alignment=TA_CENTER, fontName="Helvetica-Bold")
    ...
    elems.append(hdr_tbl([
        [Paragraph("PAYROLL MANAGEMENT SYSTEM", t_s)],
    ...
    att_data = [
        ...
        row2("OT Hours",     f"{pr.total_ot_hours:.2f}",
             "Less Hours",   f"{pr.total_less_hours:.2f}"),
        row2("Out Hours (manual)", f"{pr.total_out_hours:.2f}",
             "Gross Salary",       f"₹ {pr.gross_salary:,.2f}"),
    ]
    ...
    earn_hdr = Table(
        [[Paragraph("EARNINGS", sec_s), Spacer(3,1), Paragraph("DEDUCTIONS", sec_s)]],
    ...
    def money(v):
        return Paragraph(f"₹ {v:,.2f}", ps("MR", fontSize=8, alignment=TA_RIGHT))

    earns = [("Basic / Gross Pay", pr.gross_salary),
             ("Overtime Pay",       pr.extra_pay),
             ("Incentives",         pr.incentives)]
    deds  = [("Less Hrs Deduction", pr.less_deduction),
    ...
    ed_tbl.setStyle(TableStyle([
        ...
        ("TOPPADDING",(0,0),(-1,-1),4), ("BOTTOMPADDING",(0,0),(-1,-1),4),
        ("LEFTPADDING",(0,0),(-1,-1),5),
        ("RIGHTPADDING",(1,0),(1,-1),5), ("RIGHTPADDING",(3,0),(3,-1),5),
    ]))
    elems.append(ed_tbl); elems.append(Spacer(1, 4*mm))

    net_tbl = Table(
        [[Paragraph("NET PAY", net_s), Paragraph(f"₹ {pr.net_pay:,.2f}", net_s)]],
