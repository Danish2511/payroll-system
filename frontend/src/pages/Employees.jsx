...
import { Modal, PageLoader, Alert, EmptyState, fmtCurrency } from '../components/UI'
import { Plus, Search, Pencil, Trash2, Users, Loader2, RefreshCw } from 'lucide-react'
import { useForm } from 'react-hook-form'

const DESIG = ['RMO','AC','REC','Nurse','Pharmacist','Lab Tech','Admin','Receptionist','Support','Staff','Other']
const DEPTS = ['Medical','Nursing','Reception','Admin','Operations','Laboratory','Pharmacy','Support']
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input className="input" placeholder="E001" {...register('employee_code', { required: 'Required' })} />
          {errors.employee_code && <p className="text-red-400 text-xs mt-1">{errors.employee_code.message}</p>}
        </div>
        <div>
          <label className="label">Full Name *</label>
          <input className="input" placeholder="Dr. John Doe" {...register('name', { required: 'Required' })} />
        ...
        <div>
          ...
          <input type="number" className="input" placeholder="31200"
            {...register('monthly_salary', { required: 'Required', valueAsNumber: true, min: 0 })} />
          {errors.monthly_salary && <p className="text-red-400 text-xs mt-1">{errors.monthly_salary.message}</p>}
        </div>
        <div>
          <label className="label">Standard Hours / Day</label>
          <input type="number" step="0.5" className="input" defaultValue={8}
            {...register('standard_hours', { valueAsNumber: true })} />
          <p className="text-slate-600 text-xs mt-1">8h normal staff, 12h for RMO/24hr duty</p>
        </div>
        <div>
        ...
        <div>
          ...
          <input type="email" className="input" placeholder="doctor@clinic.com" {...register('email')} />
        </div>
        <div>
        ...
        <div className="col-span-2">
          <div className="flex items-center gap-4 bg-slate-800 rounded-xl p-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" className="w-4 h-4 rounded" {...register('pf_applicable')} />
              <span className="text-sm text-slate-300">PF Applicable</span>
            </label>
            <div className="flex items-center gap-2">
              <label className="text-xs text-slate-400">PF %:</label>
              <input type="number" step="0.01" className="input w-24" defaultValue={0}
...
export default function Employees() {
  ...
  const handleSave = async (data) => {
    ...
    try {
      ...
      setSuccess(modal !== 'create' ? 'Employee updated!' : 'Employee created!')
      setModal(null); load()
    } catch (e) {
  ...
  const handleDelete = async (emp) => {
    if (!confirm(`Delete ${emp.name}? This will remove all their attendance and payroll data.`)) return
    try {
      await deleteEmployee(emp.id)
      setSuccess('Employee deleted'); load()
    } catch (e) { setError(e.response?.data?.detail || 'Error deleting') }
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">{employees.length} registered</p>
        </div>
      ...
      <div className="relative max-w-xs">
        ...
        <input className="input pl-8 text-sm" placeholder="Search by name or code…"
      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr>
                {['Code','Name','Designation','Department','Salary','Std Hrs','Status','Actions'].map(h => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="text-center py-12"><Loader2 className="animate-spin inline text-blue-400" size={24} /></td></tr>
              ) : employees.length === 0 ? (
                <EmptyState message="No employees found. Add your first employee →" />
              ) : employees.map(emp => (
                <tr key={emp.id} className="hover:bg-slate-800/30 transition-colors">
                  <td className="td font-mono text-xs text-slate-400">{emp.employee_code}</td>
      ...
      {modal && (
        <Modal
          title={modal === 'create' ? 'Add New Employee' : `Edit — ${modal.name}`}
